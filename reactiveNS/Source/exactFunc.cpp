...
void updateExact(const int nSteps, const double dx, const int dim, std::ofstream &exact, Vector <Vector<double> >& arrExact)
{
    ...
    for (i = 2; i < (nSteps + 2); i++)
    {
        ...
        // Sample the solution
        // We have the star-state pressure and velocity, all we need is the star-state density to find the conservative flux at the interface
        ...
        // set y-velocity to zero for exact solution for 1-D test case
        ...
        exact << x << " " << rhoExact << " " << vxExact << " " << pExact << " " << epsExact << std::endl;
