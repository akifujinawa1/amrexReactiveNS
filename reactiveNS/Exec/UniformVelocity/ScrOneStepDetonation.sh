...
do
    for j in "${tests[@]}"
    do
	    mpirun -np 4 ./main1d.gnu.MPI.ex inputs amr.n_cell = "$i" "$i" "$i" euler = 1 viscous = 1 source = 2 Da = 50\
        enIC = "$j" geometry.prob_lo = 0.0 0.0 0.0 geometry.prob_hi = 1.0 1.0 1.0 adv.cfl = 0.5 \
        amr.max_level = 4 amr.ref_ratio = 2 2 4 4 amr.probin_file = prbn/detonation amr.plot_int = 30\
