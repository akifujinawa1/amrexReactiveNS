...
do
    for j in "${tests[@]}"
    do
	    mpirun -np 32 ./main1d.gnu.MPI.ex inputs amr.n_cell = 512 0 0 euler = 0 viscous = 1 source = 0 particle = 2 Da = 1\
        enIC = "$j" geometry.prob_lo = 0.0 0.0 0.0 geometry.prob_hi = 0.00512 0.0 0.0 adv.cfl = 0.8 adv.fourier = 0.2\
        ...
        TpInitial = "$i" TgInitial = 300 dp0 = 0.000010 Nsub = 20
    done
done
