...
void updateExact(const int nSteps, const double dx, const int dim, std::ofstream &exact, Vector <Vector<double> >& arrExact)
{
    ...
    double YO2Lstar, YO2Rstar, YO2Lfan, YO2Rfan, YO2L, YO2R;
    double YN2Lstar, YN2Rstar, YN2Lfan, YN2Rfan, YN2L, YN2R;
    double x0,xEnd,tEnd,xJump;
    int iter;

    amrex::Vector<double> RPLeftRight(16);
    ...
    RPLeftRight = setIC(dim,0,1,0,1);  //need to fix this
    ...
    for (i = 2; i < (nSteps + 2); i++)
    {
        ...
        if (pattern == 1 || pattern == 2)
        {
            ...
            YO2Lstar = YO2L * (g1 + g2) / (g1 * g2 + 1);
            YN2Lstar = YN2L * (g1 + g2) / (g1 * g2 + 1);
            sL = vL - mfluxL / rhoL;
        }
        ...
        else
        {
            ...
            YO2Lfan = YO2L * pow(g1, 1 / Gamma);
            YN2Lfan = YN2L * pow(g1, 1 / Gamma);
            aLstar = aL * pow(g1, g4);
            sHL = vL - aL;
            sTL = vStar - aLstar;
        }
        ...
        if (pattern == 1 || pattern == 4)
        {
            ...
            YO2Rstar = YO2R * (g7 + g2) / (g7 * g2 + 1);
            YN2Rstar = YN2R * (g7 + g2) / (g7 * g2 + 1);
            sR = vR + mfluxR / rhoR;
        }
        ...
        else
        {
            ...
            YO2Rfan = YO2R * pow(g7, 1 / Gamma);
            YN2Rfan = YN2R * pow(g7, 1 / Gamma);
            aRstar = aR * pow(g7, g4);
            sHR = vR + aR;
            sTR = vStar + aRstar;
        }        
        ...
        switch (pattern)
        {
            case 1: // shock-contact-shock
            {
                ...
                else if (sL <= xOverT && xOverT < vStar)
                {
                    ...
                    primBound[1] = vStar;
                    primBound[3] = p0;
                    primBound[4] = YO2Lstar;
                    primBound[5] = YN2Lstar;
                }
                else if (vStar <= xOverT && xOverT < sR)
                {
                    ...
                    primBound[1] = vStar;
                    primBound[3] = p0;
                    primBound[4] = YO2Rstar;
                    primBound[5] = YN2Rstar;
                }
                else if (xOverT >= sR)
                {
                    primBound = primR;
            ...
            case 2: // shock-contact-rarefaction
            {
                ...
                else if (sTR <= xOverT && xOverT < sHR)
                {
                    ...
                    primBound[4] = YO2L * pow(g5 - (g2 / aR) * (vR - xOverT), g8);
                    primBound[5] = YN2L * pow(g5 - (g2 / aR) * (vR - xOverT), g8);
                }
                else if (xOverT >= sHR)
                {
                    primBound = primR;
            ...
            case 3: // rarefaction-contact-rarefaction
            {
                ...
                else if (sHL <= xOverT && xOverT < sTL)
                {
                    ...
                    primBound[4] = YO2L * pow(g5 + (g2 / aL) * (vL - xOverT), g8);
                    primBound[5] = YN2L * pow(g5 + (g2 / aL) * (vL - xOverT), g8);
                }
                ...
                else if (sTR <= xOverT && xOverT < sHR)
                {
                    ...
                    primBound[1] = g5 * (-aR + (1 / g8) * vR + xOverT);
                    primBound[3] = pR * pow(g5 - (g2 / aR) * (vR - xOverT), g8 * Gamma);
                    primBound[4] = YO2R * pow(g5 - (g2 / aR) * (vR - xOverT), g8 * Gamma);
                    primBound[5] = YN2R * pow(g5 - (g2 / aR) * (vR - xOverT), g8 * Gamma);
        ...
        double YO2Exact = primBound[4];
        double YN2Exact = primBound[5];
        ...
        // double energyExact = energy(rhoExact,vxExact,vyExact,TgExact);
        // double epsExact    = specIntEner(rhoExact,vxExact,vyExact,energyExact);
        exact << x << " " << rhoExact << " " << vxExact << " " << pExact << " " << YO2Exact << " " << YN2Exact << std::endl;
    }
