...
void
AmrLevelAdv::initParticles (const MultiFab& S_new, const double& xDisc, const double& mFe0, \
                            const double& mFeO0, const double& mFe3O40)
{
    const int lev = 0;
    Real patch = 0;
    int totalParIter = 0;
    ...
    //    for (MFIter mfi = MakeMFIter(lev); mfi.isValid(); ++mfi)
    {
        ...
        for (int i = 0; i < Np; i++){
            ...
            p.idata(IntData::regime) = 0;         // Store the combustion regime of each particle here
            p.idata(IntData::lock)   = 0;         // Store the particle position lock flag here
