...
int      AmrLevelAdv::NUM_STATE       = 4;  // set this to 7 to store vector of conserved variables for the reactive N-S eqns. -2023W2
...
AmrLevelAdv::variableSetUp ()
{
  ...
  desc_lst.setComponent(Phi_Type, 3, "energy", bc, 
			                  StateDescriptor::BndryFunc(nullfill));
  // desc_lst.setComponent(Phi_Type, 4, "yH2", bc, 
	// 		                  StateDescriptor::BndryFunc(nullfill));
  // desc_lst.setComponent(Phi_Type, 5, "yO2", bc, 
	// 		                  StateDescriptor::BndryFunc(nullfill));
  // desc_lst.setComponent(Phi_Type, 6, "yH2O", bc, 
	// 		                  StateDescriptor::BndryFunc(nullfill));                      
}
...
//
//  Euler (MUSCL) and diffusive (centred) fluxes are calculated first, followed by reactive source terms within the gas-phase.
//  Particle information is then updated, and Lagrangian source terms are computed for corresponding cells in the gas.
//
Real
                      ...
                      int  ncycle)
{
  ...
  for (int d = 0; d < amrex::SpaceDim ; d++)   
  {
    ...
    for (MFIter mfi(Sborder, true); mfi.isValid(); ++mfi)
    {
      ...
      // std::cout << "before defining new arr" << std::endl;
            ...
            for(int k = lo.z; k <= hi.z; k++)
            {
              for(int j = lo.y; j <= hi.y; j++)
              {
                for(int i = lo.x; i <= hi.x+gCells; i++)
                {
                  ...
                  // std::cout << "post u0" << std::endl;

                  double w = 0;
                  reconstruct(boundLslice,boundRslice,slopeCells,u0,w,dx[d],dt,d);

                  // std::cout << "post recon" << std::endl;

                  for(int h = 0; h < NUM_STATE; h++) // fill matrix u0 with values from arr to calculate slope ratio. -2023W2
                  {
                    boundLx(i-1,j,k,h) = boundLslice[h];
                    boundRx(i-1,j,k,h) = boundRslice[h];
                  }

                  // std::cout << "post boundLxRx assign" << std::endl;
            ...
            for(int k = lo.z; k <= hi.z; k++)
            {
              for(int j = lo.y; j <= hi.y; j++)
              {
                ...
                {                  
                  ...
                  for(int h = 0; h < NUM_STATE; h++)
                  {
                    qL[h] = boundRx(i-1,j,k,h);
                    qR[h] = boundLx(i,j,k,h);
                  }

                  // std::cout << "post qLqR" << std::endl;
                  ...
                  // std::cout << "post HLLCflux" << std::endl;
                  ...
                  // std::cout << "post fluxArr" << std::endl;
                }
              }
            }
            ...
            for(int k = lo.z; k <= hi.z; k++)
            {
              for(int i = lo.x; i <= hi.x; i++)
              {
                for(int j = lo.y; j <= hi.y+gCells; j++)
                {
                  ...
                  for(int h = 0; h < NUM_STATE; h++) // fill matrix u0 with values from arr to calculate slope ratio. -2023W2
                  {
                    boundLy(i,j-gCells+1,k,h) = boundLslice[h];
                    boundRy(i,j-gCells+1,k,h) = boundRslice[h];
                  }
            ...
            for(int k = lo.z; k <= hi.z; k++)
            {
              for(int i = lo.x; i <= hi.x; i++)
              {
                ...
                {                
                  ...
                  for(int h = 0; h < NUM_STATE; h++)
                  {
                    qL[h] = boundRy(i,j-gCells+1,k,h);
                    qR[h] = boundLy(i,j-gCells+2,k,h);
                  }
                  // Call HLLCflux using the left and right states, qL and qR, and the flux direction
                  // tracked by the for loop variable 'd' to compute the correct fluxes. d=0 corresponds 
                  // to the x-direction, and d=1 corresponds to the y-direction.  -2023W2
